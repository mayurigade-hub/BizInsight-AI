...
app.add_middleware(
    ...
    # Restrict CORS to only the methods and headers actually used by the
    # Streamlit frontend. Allowing all methods/headers is overly permissive
    # and enables DELETE/PUT/etc. from browser tabs which the UI never uses.
...
class ChatRequest(BaseModel):
    ...
    session_id: Optional[str] = (
        None  # An optional session ID for conversational context
    )
    use_memory: bool = (
        False  # Whether to use conversational memory (True for follow-up questions, False for standalone queries)
    )
...
class ChatResponse(BaseModel):
    ...
    sources: List[
        str
    ]  # A list of source documents that the AI used to generate its answer
    session_id: Optional[str] = (
        None  # Echo back the session ID if provided, so the frontend can maintain conversation state
    )
...
class SyncRequest(BaseModel):
    documents: List[
        Dict[str, Any]
    ]  # A list of documents to be added to the vector store, each with 'page_content' and optional 'metadata' and 'id'
...
class HealthResponse(BaseModel):
    ...
    vector_count: Optional[int] = (
        None  # The number of vectors currently in the vector store, useful for monitoring and debugging
    )
...
async def health_check():
    try:
        count = (
            chain_manager.vector_store_manager.vectorstore._collection.count()
        )  # Access the internal collection to get the count of vectors
...
def chat(request: Request, body: ChatRequest):
    ...
    if any(
        ...
        for word in ["issue", "problem", "bad", "complaint", "wrong", "broken"]
    ):
    ...
    # If they ask about positive things, only filter for positive reviews in ChromaDB.
        ...
        word in question_lower
        for word in ["good", "great", "best", "love", "awesome", "perfect"]
    ):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if body.use_memory and body.session_id:
                chain = chain_manager.get_conversational_chain(
                    body.session_id, search_filter=search_filter
                )
                ...
                if answer:
                    try:
                        ...
                        logger.warning(
                            "Could not persist chat turn for %s: %s",
                            body.session_id,
                            exc,
                        )
            else:
            ...
            ):  # If we've exhausted all retry attempts, we log the error and return a 502 Bad Gateway response indicating that the AI provider is currently unstable.
