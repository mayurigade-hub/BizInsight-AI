...
# NOTE:
# This module was previously implemented using the `fpdf` library.
# The project currently generates PDFs via `reportlab` in app.py.
# The `create_pdf()` entrypoint in this file is unused and keeping `fpdf`
# as a dependency forces users to install a heavyweight library unnecessarily.
# The FPDF-based implementation has therefore been removed.
from datetime import datetime


def create_pdf(total, positive, negative, chart_path):
    raise RuntimeError(
        "FPDF-based PDF generation was removed because this module's create_pdf() is unused. "
        "The application now uses reportlab for PDF generation."
    )
    ...
    pdf.cell(0, 12, "Business Health Overview", ln=True, fill=True)
    pdf.ln(10)
    ...
    pdf.cell(0, 10, "Generated by BizInsight AI", ln=True, align="R")
