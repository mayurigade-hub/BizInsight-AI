...
def prepare_daily_sentiment(df):
    ...
    if len(daily) == 1:
        ...
        demo_sentiments = [
            np.clip(
                last_sentiment + np.random.uniform(-0.15, 0.15),
                -1,
                1
            )
            for _ in range(30)
...
def forecast_sentiment(df, forecast_days=7):
    ...
    daily_df = prepare_daily_sentiment(df)

    n_lags = 7

    if len(daily_df) < n_lags + 1:
        raise ValueError(
            f"At least {n_lags + 1} days of sentiment data are required."
        )
